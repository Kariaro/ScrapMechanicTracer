...
class CodeSyntaxTreeUnit {
		...
		switch(op.getOpcode()) {
			case PcodeOp.CALL: {
				...
				switch(location.getLabel()) {
					...
					default: {
						Varnode output = op.getOutput();
						Logger.log("::::: Calling (%s) -> out %s", location, output);
						
						Varnode[] inputs = utils.getInputs(op, node);
						for(int i = 1; i < inputs.length; i++) {
							Logger.log(":::::     %2d: %-30s", i, inputs[i]);
						}
					}
				}
			}
	...
	public void process_basic(NodeFunction node, PcodeOp op) {
		...
		// Logger.log("Non external function: %s", op);
	...
	private void luaL_checklstring(PcodeOp op) {
		...
		if(arg.isConstant()) {
			...
			Logger.log("1) luaL_checklstring: %s", arg);
			Logger.log("2) luaL_checklstring: %s", arg.getDef());
		}
	}
	...
	private void luaL_checktype(PcodeOp op) {
		...
		if(arg.isConstant() && type.isConstant()) {
			...
			Logger.log("::::: Calling (luaL_checktype) -> out ?");
			for(int i = 1; i < inputs.length; i++) {
				Logger.log(":::::     %2d: %-30s %-30s", i, inputs[i], inputs[i].getDef());
			}
		}
	}
	...
	private void luaL_checkudata(PcodeOp op) {
		...
		if(debug) {
			Logger.log("::::: Calling (luaL_checkudata) -> out ?");
			for(int i = 1; i < inputs.length; i++) {
				Logger.log(":::::     %2d: %-30s\n", i, inputs[i]);
			}
		}
...
//		Logger.log("lua_pushfstring: \"" + message + "\"");
//		
//		for(int i = 1; i < inputs.length; i++) {
//			Logger.log("    %2d: %s", i, inputs[i]);
//		}
//	}
	...
	private void luaL_argerror(PcodeOp op) {
		...
		for(int i = 1; i < inputs.length; i++) {
			...
			//Logger.log(":::::     %2d: %-30s %-30s", i, input, input.getDef());
		...
		
		{
			...
			if(format.endsWith(" expected, got %s")) {
				...
				Logger.log("======================================================================");
				Logger.log("Calling (LUA51.DLL::luaL_argerror) -> out ?");
				Logger.log(":: format = %s", format);
...
//			Logger.log("======================================================================");
//			Logger.log("luaL_error: node_format was not constant?");
//			Logger.log("op: %s", op);
//			Logger.log("node: %s", node_format);
			return;
		}
	...
	private void resolveUnsafe(String format, PcodeOp op, Varnode[] inputs) {
		...
		*/
	 	if(format.startsWith("Invalid projectile attack source. Expected a Player or Unit, but received:")) {
	 		...
	 		Logger.log("======================================================================");
	 		Logger.log("luaL_error: Unknown error message '%s'", format);
