 ...
 * @date 2020-11-22
 */
public class ScrapMechanicAnalyser {
	...
	public boolean startAnalysis() {
		...
		for(SMDefinition object : objects) {
			if(object.getName() == null) {
				Logger.log("[SMDefinition] Was null! %s", object);
				continue;
			}
			...
			Logger.log("%s: %s, %s", object, name, factory.getAddress(object.getName()));
			SMClass clazz = table.createClass(name);
			if(clazz == null) {
				Logger.log("[SMClass] was null!!");
				continue;
			}
